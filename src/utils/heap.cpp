 ...
 */
/**
 * @file src/utils/heap.cpp
 * @author Robin Coutelier
 *
 * @brief This file is part of the NapSAT solver. If implements a heap data structure for integer keys with double activities.
 */
#include "heap.hpp"
...
void napsat::utils::heap::print()
{
  ...
  for (unsigned i = 0; i < bfs_queue.size(); i++) {
    ...
    unsigned j = bfs_queue[i];
    if (j < _heap.size()) {
      std::cout << _heap[j].to_string() << " (" << _activity[_heap[j]] << ") ";
    }
    else {
      std::cout << "X ";
    }
