...
namespace sentinel::wrapper
{
  ...
  inline bool lock_assumption(sentinel::SATSentinel* sentinel, napsat::Tlit blocker) {
    return sentinel::lock_assumption(sentinel, convert(blocker));
  }
  inline bool unlock_assumption(sentinel::SATSentinel* sentinel, napsat::Tlit blocker) {
    return sentinel::unlock_assumption(sentinel, convert(blocker));
  }
